class IdentitySlack < Identity
  def self.find_or_initialize_with_id(id, integration)
    ...
    unless identity
      if another_identity = self.joins(:integration).where(integrations: {team_id: integration.team}).find_by(primary_key: id)
        identity = self.new(integration_id: integration.id)
        identity.attributes = another_identity
                              .attributes
                              .reject{|k, v| k.to_sym.in?([:id, :integration_id, :user_team_id, :created_at, :updated_at]) }
      else
